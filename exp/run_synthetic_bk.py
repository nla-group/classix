...
def rn_sklearn_benchmark():
    ...
    # ============
    # Store scores
    clustering_algorithms_scores_ar = {
        # 'MiniBatch\nk-means': [],
        'k-means++' : [],
        'Affinity\nPropagation' : [],
        'MeanShift': [],
    ...
    params_dbcu = {'tols': [0.25, 0.18, 0.09, 0.12, 0.15, 1],
                  'sorting': ['pca', 'pca', 'pca', 'pca', 'pca', 'pca'],
                  'minPts':[0, 0, 11, 11, 0, 0]
    }
    ...
    with threadpool_limits(limits=1, user_api='blas'):
        for i_dataset, (dataset, algo_params) in enumerate(_datasets):
            ...
            #     params_dbcu['sorting'][i_dataset],
            #     np.round(params_dbcu['tols'][i_dataset],1)
            # ))
            # if i_dataset != 2:
            #     continue
            ...
            classix = CLASSIX(sorting=params_dbcu['sorting'][i_dataset],
                        radius=params_dbcu['tols'][i_dataset], 
                        group_merging='distance', verbose=0,
                        minPts=params_dbcu['minPts'][i_dataset]
                      )
            ...
            for name, algorithm in clustering_algorithms:
                ...
                for _iter in range(sample_size):
                    ...
                    if name == 'CLASSIX':
                        classix_dist_nr = classix_dist_nr + algorithm_clone.dist_nr

                if i_dataset == 5:
                    y = np.ones(len(y_pred))
                
                classix_dist_nr = classix_dist_nr / sample_size
                ar = metrics.adjusted_rand_score(y, y_pred)
                ami = metrics.adjusted_mutual_info_score(y, y_pred)

                clustering_algorithms_scores_ar[str(name)].append(ar)
                ...
                plt.yticks(())
                if name == "CLASSIX":
                    plt.text(.01, .01, ('%.2f' % (classix_dist_nr/len(y))).lstrip('0'),
    ...
    arcsv.to_csv('fresults/arcsv_en.csv', index=False)
    amicsv.to_csv('fresults/amicsv_en.csv', index=False)