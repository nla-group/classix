...
def rn_cluster_shape():
    ...
    den_minPts = [5, 5, 9, 2, 0, 8, 3, 0]
    dist_tols = [0.1, 0.1, 0.025, 0.2, 0.3, 0.15, 0.15, 0.25]
    dist_minPts = [0, 1, 20, 8, 0, 9, 3, 4]
    ...
    with threadpool_limits(limits=1, user_api='blas'):
        for i in range(len(shape_sets)):
            ...
            for _iter in range(sample_size):
                ...
                st = time.time()
                _data = (data[[0,1]] - data[[0,1]].mean(axis=0)) / data[[0,1]].std(axis=0)
                _hdbscan = hdbscan.HDBSCAN(min_cluster_size=min_cluster_size[i])
    ...
    for i_dataset in range(len(shape_sets)):
        ...
        for name, results in clustering_algorithms_results:
            ...
            plt.xticks(())
            plt.yticks(())
            if name == "CLASSIX - density":
                plt.text(.01, .01, ('%.2f' % (classix_den_nr_dist[i_dataset]/len(data))).lstrip('0'),
                         transform=plt.gca().transAxes, size=24,
                         horizontalalignment='left')
            elif name == "CLASSIX - distance":
                plt.text(.01, .01, ('%.2f' % (classix_dist_nr_dist[i_dataset]/len(data))).lstrip('0'),
                         transform=plt.gca().transAxes, size=24,
                         horizontalalignment='left')
            if i_dataset == 0:
                plt.title(name, size=27)
                
            plt.text(.99, .01, ('%.2fs' % (clustering_algorithms_time[name][i_dataset])).lstrip('0'),
                     transform=plt.gca().transAxes, size=24,
                     horizontalalignment='right')

            plot_num = plot_num + 1
            
    plt.savefig('fresults/shape_sets_general'+'.pdf', bbox_inches='tight')
    # plt.show()
    ...
    SCORE_STORE.to_csv('fresults/shape_r.csv', index=False)
    

    
    
def shape_index_plot():
    data = pd.read_csv("fresults/shape_r.csv")
    ...
    plt.tight_layout()
    plt.savefig("fresults/shape_sets_ri.pdf", bbox_inches='tight')
    # plt.show()
