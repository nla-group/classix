...
def count_distance():
    ...
    fname = ['fresults/nr_dist_centers_num.csv', 'fresults/centers_num_ln.csv',
             'fresults/centers_num_ami.csv', 'fresults/centers_num_ari.csv', 
             'fresults/nr_dist_fdim_num.csv', 'fresults/fdim_num_ln.csv', 
             'fresults/fdim_num_ami.csv', 'fresults/fdim_num_ari.csv',
             'fresults/nnr_dist_centers_num.csv', 'fresults/ncenters_num_ln.csv',
             'fresults/ncenters_num_ami.csv', 'fresults/ncenters_num_ari.csv', 
             'fresults/nnr_dist_fdim_num.csv', 'fresults/nfdim_num_ln.csv', 
             'fresults/nfdim_num_ami.csv', 'fresults/nfdim_num_ari.csv']
...
def rn_sort_early_stp():
    ...
    np.save('fresults/norm_orthant_true.npy', norm_orthant_true)
    np.save('fresults/norm_orthant_false.npy', norm_orthant_false)
    np.save('fresults/norm_mean_true.npy', norm_mean_true)
    np.save('fresults/norm_mean_false.npy', norm_mean_false)
    np.save('fresults/pca_dist_true.npy', pca_dist_true)
    np.save('fresults/pca_dist_false.npy', pca_dist_false)
...
def rn_sort_plot1():
    ...
    plt.savefig('fresults/sorting_comp_1.pdf', bbox_inches='tight')
    # plt.show()
    
    
    
def rn_sort_plot2():
    nr_dist_centers_num = pd.read_csv("fresults/nr_dist_centers_num.csv").iloc[:500]
    nnr_dist_centers_num = pd.read_csv("fresults/nnr_dist_centers_num.csv").iloc[:500]
    plt.figure(figsize=(8,6))
    plt.rcParams['axes.facecolor'] = 'white'
    ...
    plt.savefig('fresults/sorting_comp_2.pdf', bbox_inches='tight')
    # plt.show()