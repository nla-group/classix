...
The documentation mainly contains five chapters about user's guidance and one chapter for API reference. The first chapter demonstrate a quick introduction about installment of CLASSIX and deployment. The second chapter compare CLASSIX with other sought-after clustering algorithms with respect to speed and accuracy on built-in data. The third chapter illustrates a vivid tutorial for density and distance merging application. The fourth chapter illustrate the intepretable clustering result obtained from CLASSIX. The final chapter demonstrate how to use CLASSIX to find outliers in data. The documentaion is still under construction, any suggestions from users are appreciated, please be free send us email for any questions on CLASSIX.   
