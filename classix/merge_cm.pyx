...
cpdef distance_merge_set(double[:, :] data, double[:, :] splist, double radius, double scale=1.5):
    """
    Implement CLASSIX's merging with disjoint-set data structure for the merging.
...
cpdef distance_minpts_no_merge(double[:, :] data, list labels, np.ndarray[np.int64_t, ndim=1] sp_cluster_labels, np.ndarray[np.int32_t, ndim=1] cs,
                       long[:, :] splist,  int minPts, double[:] half_nrm2):
