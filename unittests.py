...
from classix.merge_md import merge_manhattan, bfs_shortest_path as bfs_md
from classix.merge_td import merge_tanimoto, bfs_shortest_path as bfs_td
import platform
...
class TestClassix(unittest.TestCase):
    ...
    def test_aggregate_manhattan(self):
        """Test Manhattan distance aggregation module."""
        checkpoint = 1
        try:
            X, _ = data.make_blobs(n_samples=500, centers=3, n_features=2, random_state=42)
            result = aggregate_manhattan(X, radius=0.5)
            
            # Verify all required keys exist
            self.assertIn('labels', result)
            self.assertIn('splist', result)
            self.assertIn('group_sizes', result)
            self.assertIn('ind', result)
            self.assertIn('sort_vals', result)
            self.assertIn('data_sorted', result)
            self.assertIn('nr_dist', result)
            
            # Verify shapes
            self.assertEqual(len(result['labels']), len(X))
            self.assertTrue(len(result['splist']) > 0)
            self.assertEqual(len(result['group_sizes']), len(result['splist']))
    ...
    def test_aggregate_tanimoto(self):
        ...
        try:
            ...
            # Generate binary data for Tanimoto
            X, _, _ = generate_data(num_clusters=3, pops=[5, 10, 15], d=20, n=50, flip_prob=0.1, seed=42)
            result = aggregate_tanimoto(X, radius=0.2)
    ...
    def test_merge_manhattan(self):
        ...
        try:
            ...
            # Test with mergeTinyGroups=True
            ...
            self.assertIn('group_cluster_labels', merge_result)
            self.assertIn('Adj', merge_result)
            self.assertEqual(len(merge_result['group_cluster_labels']), len(spdata))
            ...
            self.assertIn('group_cluster_labels', merge_result2)
            self.assertIn('Adj', merge_result2)
            
        except Exception as e:
            checkpoint = 0
            print(f"Manhattan merge test failed: {e}")
            
        self.assertEqual(checkpoint, 1)


    def test_merge_tanimoto(self):
        ...
        try:
            ...
            X, _, _ = generate_data(num_clusters=3, pops=[5, 10, 15], d=20, n=50, flip_prob=0.1, seed=42)
            agg_result = aggregate_tanimoto(X, radius=0.2)
            ...
            checkpoint = 0
            print(f"Tanimoto merge test failed: {e}")
            
        self.assertEqual(checkpoint, 1)


    def test_bfs_shortest_path_manhattan(self):
        """Test BFS shortest path function from merge_md module."""
        checkpoint = 1
        try:
            ...
            # Test path to self
            path_self = bfs_md(adj, 1, 1)
            self.assertEqual(path_self, [1])
            ...
            path_none = bfs_md(adj_disconnected, 0, 3)
            self.assertIsNone(path_none)
            
        except Exception as e:
            checkpoint = 0
            print(f"BFS Manhattan test failed: {e}")
            
        self.assertEqual(checkpoint, 1)


    def test_bfs_shortest_path_tanimoto(self):
        """Test BFS shortest path function from merge_td module."""
        checkpoint = 1
        try:
            # Create a simple adjacency matrix
            adj = np.array([
            ...
            # Test path to self
            path_self = bfs_td(adj, 2, 2)
            self.assertEqual(path_self, [2])
            ...
            path_none = bfs_td(adj_disconnected, 0, 3)
            self.assertIsNone(path_none)
            
        except Exception as e:
            checkpoint = 0
            print(f"BFS Tanimoto test failed: {e}")
            
        self.assertEqual(checkpoint, 1)


    def test_manhattan_clustering_integration(self):
        """Integration test for Manhattan metric in CLASSIX."""
        checkpoint = 1
        try:
            X, y = data.make_blobs(n_samples=500, centers=3, n_features=2, 
                                   cluster_std=1.0, random_state=42)
            
            # Test with different parameter combinations
            clx1 = CLASSIX(metric='manhattan', radius=0.5, minPts=10, 
                          group_merging='distance', verbose=0)
            clx1.fit(X)
            self.assertTrue(len(np.unique(clx1.labels_)) > 0)
            
            # Test with mergeTinyGroups=False
            clx2 = CLASSIX(metric='manhattan', radius=0.5, minPts=10,
                          group_merging='distance', mergeTinyGroups=False, verbose=0)
            clx2.fit(X)
            self.assertTrue(len(np.unique(clx2.labels_)) > 0)
            
            # Test predict
            predictions = clx1.predict(X[:100])
            self.assertEqual(len(predictions), 100)
            
        except Exception as e:
            checkpoint = 0
            print(f"Manhattan integration test failed: {e}")
            
        self.assertEqual(checkpoint, 1)


    def test_tanimoto_clustering_integration(self):
        """Integration test for Tanimoto metric in CLASSIX."""
        checkpoint = 1
        try:
            import sys, os
            sys.path.insert(0, os.path.dirname(__file__))
            ...
            X, y, _ = generate_data(num_clusters=3, pops=[5, 10, 15], d=20, 
                                    n=100, flip_prob=0.1, seed=42)
            
            # Test with different parameter combinations
            clx1 = CLASSIX(metric='tanimoto', radius=0.15, minPts=10,
                          group_merging='distance', verbose=0)
            clx1.fit(X)
            self.assertTrue(len(np.unique(clx1.labels_)) > 0)
            
            # Test with mergeTinyGroups=False
            clx2 = CLASSIX(metric='tanimoto', radius=0.15, minPts=10,
                          group_merging='distance', mergeTinyGroups=False, verbose=0)
            clx2.fit(X)
            self.assertTrue(len(np.unique(clx2.labels_)) > 0)
            
            # Test predict
            predictions = clx1.predict(X[:50])
            self.assertEqual(len(predictions), 50)
            
        except Exception as e:
            checkpoint = 0
            print(f"Tanimoto integration test failed: {e}")
